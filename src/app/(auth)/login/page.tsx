  ...
  return (
    <div className="w-full max-w-md bg-white dark:bg-[#0c0c0e] border border-zinc-200 dark:border-zinc-800/80 rounded-3xl shadow-xl p-8 sm:p-10 animate-in fade-in zoom-in-95 duration-500">
      <div className="flex flex-col items-center mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-zinc-900 dark:text-white">Welcome back</h1>
        <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400 mt-2 text-center">Log in to architect your databases.</p>
      </div>

      {error && (
        <div className="mb-6 p-3 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-900/50 rounded-xl text-sm font-medium text-red-600 dark:text-red-400 text-center animate-in fade-in slide-in-from-top-2">{error}</div>
      )}
      {successMsg && (
        <div className="mb-6 p-3 bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900/50 rounded-xl text-sm font-medium text-green-600 dark:text-green-400 text-center animate-in fade-in slide-in-from-top-2">{successMsg}</div>
      )}

      <form onSubmit={handleSubmit} className="space-y-5 mb-8">
        <div>
          <label className="block text-sm font-bold text-zinc-900 dark:text-zinc-100 mb-1.5">Email Address</label>
      ...
      <div className="relative flex items-center justify-center mb-8">
        ...
        <div className="relative bg-white dark:bg-[#0c0c0e] px-4 text-[10px] font-bold tracking-widest text-zinc-400 uppercase">Or continue with</div>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <button type="button" onClick={() => handleOAuthSignIn("github")} className="flex items-center justify-center gap-2 py-2.5 px-4 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all text-sm font-bold text-zinc-900 dark:text-white active:scale-95">
          <GithubIcon className="w-4 h-4" /> GitHub
        </button>
        <button type="button" onClick={() => handleOAuthSignIn("google")} className="flex items-center justify-center gap-2 py-2.5 px-4 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all text-sm font-bold text-zinc-900 dark:text-white active:scale-95">
