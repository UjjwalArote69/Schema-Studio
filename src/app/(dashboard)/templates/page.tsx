      ...
      {filteredTemplates.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 bg-zinc-50/50 dark:bg-zinc-900/20 rounded-3xl border-2 border-dashed border-zinc-200 dark:border-zinc-800">
          ...
          <p className="text-zinc-600 dark:text-zinc-400 font-medium">
            No templates found for &quot;<span className="text-black dark:text-white">{searchQuery}</span>&quot;
          </p>
          <button
            onClick={() => { setSearchQuery(""); setSelectedCategory("All"); }}
            className="mt-4 px-4 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-lg text-black dark:text-white text-sm font-bold hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors shadow-sm"
          >
            Clear all filters
          </button>
        </div>
        ...
        <div className="mt-auto pt-5 border-t border-zinc-100 dark:border-zinc-800/50 flex items-center justify-between">
          <div className="flex items-center gap-3 text-xs text-zinc-500 font-semibold uppercase tracking-wider">
            <div className="flex items-center gap-1.5" title="Tables">
              <TableIcon className="w-3.5 h-3.5" /> {template.tableCount}
            </div>
            <div className="flex items-center gap-1.5" title="Relationships">
              <Layers className="w-3.5 h-3.5" /> {template.relationCount}
            </div>
          </div>
          ...
          <button
            ...
            onClick={handleUse}
            disabled={isPending}
            className="flex items-center gap-2 px-4 py-2 bg-black hover:bg-zinc-800 dark:bg-white dark:hover:bg-zinc-200 text-white dark:text-black rounded-lg text-sm font-bold transition-all active:scale-95 shadow-sm disabled:opacity-50"
          >
            {isPending ? (
