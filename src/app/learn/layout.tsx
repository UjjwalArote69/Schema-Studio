...
import {
  Database, GraduationCap, ChevronLeft, X, Menu, ChevronDown, Search,
  Zap, Layers, Rocket,
} from "lucide-react";
...
const NAV: NavGroup[] = [
  {
    id: "beginner", title: "Beginner", icon: Zap,
    children: [
      { id: "intro", title: "What is SQL?" },
      { id: "databases-tables", title: "Databases & Tables" },
      { id: "select", title: "SELECT Basics" },
      { id: "where", title: "Filtering with WHERE" },
      { id: "order-limit", title: "Sorting & Limiting" },
      { id: "dml", title: "INSERT, UPDATE, DELETE" },
      { id: "data-types", title: "Data Types" },
    ],
  },
  {
    id: "intermediate", title: "Intermediate", icon: Layers,
    children: [
      { id: "joins", title: "JOINs" },
      { id: "aggregates", title: "Aggregate Functions" },
      { id: "groupby", title: "GROUP BY & HAVING" },
      { id: "subqueries", title: "Subqueries" },
      { id: "constraints", title: "Constraints" },
      { id: "indexes", title: "Indexes" },
      { id: "null-handling", title: "NULL Handling" },
      { id: "aliases-distinct", title: "Aliases & DISTINCT" },
    ],
  },
  {
    ...
    children: [
      ...
      { id: "functions", title: "Functions & Procedures" },
...
export default function LearnLayout({ children }: { children: React.ReactNode }) {
          ...
          <div key={group.id}>
            <button
              onClick={() => {
                ...
                scrollTo(hasChildren ? group.children![0].id : group.id);
              }}
            ...
            >
              ...
              {hasChildren && (
                <ChevronDown className={`w-3 h-3 opacity-40 transition-transform ${isExpanded ? "" : "-rotate-90"}`} />
              )}
            </button>
              ...
              <div className="ml-4 pl-3 border-l border-zinc-200 dark:border-zinc-800 space-y-0.5 mt-0.5 mb-1.5">
                {group.children!.map((child) => (
                  <button
                    ...
                    className={`w-full text-left block px-2.5 py-1.5 rounded-md text-[12.5px] transition-all ${
                      activeSection === child.id
                        ? "text-zinc-900 dark:text-white font-semibold bg-zinc-100 dark:bg-zinc-800/60"
  ...
  return (
    <LearnContext.Provider value={{ activeSection, setActiveSection }}>
        ...
        <header className="sticky top-0 z-40 h-14 border-b border-zinc-200 dark:border-zinc-800 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-xl flex items-center justify-between px-4 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <div className="p-1 bg-black dark:bg-white rounded-[6px]">
                <Database className="w-3.5 h-3.5 text-white dark:text-black" />
              </div>
            ...
            <div className="flex items-center gap-1.5 text-[11px] font-bold text-zinc-400 uppercase tracking-widest">
              ...
              <span className="hidden sm:inline">Learn SQL</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="md:hidden p-2 rounded-lg border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-900 transition-colors"
            >
        ...
        <div className="flex">
          <aside className="hidden md:flex w-[250px] lg:w-[260px] shrink-0 flex-col border-r border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950">
            <div className="sticky top-14 flex flex-col max-h-[calc(100vh-3.5rem)] overflow-hidden">
              <div className="p-3 pb-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-8 pr-8 py-2 bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-black dark:focus:ring-white transition-all text-zinc-900 dark:text-white placeholder:text-zinc-400"
                  />
                  {searchQuery && (
                    <button
              ...
              <div className="flex-1 overflow-y-auto px-3 pb-6 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-zinc-300 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-700 [&::-webkit-scrollbar-thumb]:rounded-full">
                {navTree}
              </div>
            </div>
          </aside>
            ...
            <>
              ...
              <aside className="fixed inset-y-0 left-0 z-40 w-72 bg-white dark:bg-zinc-950 border-r border-zinc-200 dark:border-zinc-800 flex flex-col md:hidden animate-in slide-in-from-left duration-200">
                <div className="h-14 flex items-center justify-between px-4 border-b border-zinc-200 dark:border-zinc-800">
                  ...
                  <button onClick={() => setMobileNavOpen(false)} className="p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-white">
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto p-3">
                  <div className="relative mb-3">
                    ...
                    <input
                      ...
                      onChange={(e) => setSearchQuery(e.target.value)}
                      placeholder="Search topics..."
                      className="w-full pl-8 pr-3 py-2 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-black dark:focus:ring-white text-zinc-900 dark:text-white placeholder:text-zinc-400"
                    />
                  </div>
                  {navTree}
                </div>
